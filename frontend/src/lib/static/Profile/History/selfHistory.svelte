<script lang="ts">
    ...
    async function ParsePongGame(data : any) {
        let player1 : any = await userData(data.player1 == state.user?.id ? data.player1 : data.player2);
        let player2 : any = await userData(data.player1 != state.user?.id ? data.player1 : data.player2);
        let scoreMe = 0;
        let scoreOpponent = 0;
    ...
    async function ParseShooterGame(data : any) {
        let player1 : any = await userData(data.players[0] == state.user?.id ? data.players[0] : data.players[2]);
        let player2 : any = await userData(data.players[0] != state.user?.id ? data.players[0] : data.players[2]);
