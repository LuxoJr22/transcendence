        ...
        {#each gamesHistory as game}
            {#if game.gamemode != "Shooter" && historyToDisplay == 'pong'}
                <div class="row border rounded match my-1 bg-dark text-truncate {game.winner == userId ? 'border-primary' : 'border-danger'}">
                    ...
                    <div class="d-flex">
                        <p class="col-4 mb-0" style="color:grey;">{game.date}</p>
                        <div class="col-4 d-flex justify-content-center">
                            <p class="game-title text-light text-center mb-0">{game.gamemode.toUpperCase()}</p>
                            {#if game.type == 'normal'}
                                <i class="bi bi-people-fill text-light ms-1 mb-0" title='Public Game'></i>
                            {:else if game.type == 'private'}
                                <i class="bi bi-person-fill-lock text-light ms-1 mb-0" title='Private Game'></i>
                            {:else if game.type == 'tournament'}
                                <i class="bi bi-trophy-fill text-light ms-1 mb-0" title='Tournament Game'></i> 
                            {/if}
                        </div>
                        <p class="col-4 text-end  mb-0" style="color:grey;">{game.hours}</p>
                    </div>
                </div>
