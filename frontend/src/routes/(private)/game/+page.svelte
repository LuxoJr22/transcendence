<script lang= "ts">
	...
	import { Player } from "$lib/assets/games/pong/player.js";
	import { Bot } from "$lib/assets/games/pong/bot.js";
	import { shade } from "$lib/assets/games/pong/watershader";
	const popUrl = new URL('$lib/assets/games/pong/public/pop.glb', import.meta.url).href;
	const pirUrl = new URL('$lib/assets/games/pong/public/pir.glb', import.meta.url).href;
	const styUrl = new URL('$lib/assets/games/pong/public/sty.glb', import.meta.url).href;
	const bueUrl = new URL('$lib/assets/games/pong/public/bue.glb', import.meta.url).href;
	let canvas;
	var scoring = 0;
	...
	if (canvas){
		onMount(() => { (async () => {
			...
			const g = await loader.loadAsync(popUrl);
			...
			const gltf = await loader.loadAsync(pirUrl);
			...
			const gl = await loader.loadAsync(styUrl);
			...
			const bouee = await loader.loadAsync(bueUrl);
